...
from ..tools.csrc import furthest_point_sampling as fps
...
class Group(nn.Module):
    ...
    def morton_sorting(self, xyz, center):
        batch_size, _, _ = xyz.shape  # (B, N, 3)
        all_indices = []
    ...
    def forward(self, xyz):
        ...
        batch_size, num_points, _ = xyz.shape
        # fps the centers out
        center_idx = fps(xyz, self.num_group)  # B G 3
        center_idx = center_idx[...,None].repeat(1,1,3)
        center = torch.gather(xyz, dim=1, index=center_idx)
...
class PointGPT(nn.Module):
    ...
    def get_point_tokens(self, pts:torch.Tensor):
        neighborhood, center = self.group_divider(pts)
        features = self.GPT_Transformer.encoder(neighborhood)
...
class PointTransformer(nn.Module):
    ...
    def extraction(self, pts:torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """extract point features

        Args:
            pts (torch.Tensor): (B, N ,3) normalized point cloud

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: center (B, K, 3), encoded_features (B, K, D)
        """
        neighborhood, center = self.group_divider(pts)
        ...
        return center, encoded_features
