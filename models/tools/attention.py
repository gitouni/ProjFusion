...
class ViTEncoder(nn.Module):
    def __init__(self, modelname:str, image_hw:Tuple[int,int], cache_dir="/home/ouni/.cache/torch/hub/",
                source:Literal['github','local']='local', reshape:bool=False, freeze:bool=False):
        super().__init__()
        self.freeze = freeze
...
# Modified From https://github.com/lucidrains/vit-pytorch/blob/main/vit_pytorch/na_vit.py
class AttentionUnNorm(nn.Module):
    def __init__(self, input_dim:int, heads = 8, dim_head = 64, dropout = 0., **argv):
        super().__init__()
        inner_dim = dim_head *  heads
        self.heads = heads
    ...
    def forward(
        ...
        **argv
        ...
        q: torch.Tensor = self.q_norm(q)  # (B, H, N, d)
        k: torch.Tensor = self.k_norm(k)  # (B, H, M, d)
        if exists(attn_mask):
            if attn_mask.ndim == 2:
                attn_mask = attn_mask[None, None, :, :]
            elif attn_mask.ndim == 3:
...
class AttentionSplit(nn.Module):
    def __init__(self, input_dim: int, heads: int, dim_head: int, coord_dim: int, dropout = 0., **argv):
        super().__init__()
        inner_dim = dim_head *  heads
        self.heads = heads
        self.dim_head = dim_head
        self.norm = LayerNorm(input_dim + coord_dim)  # union normalized
        self.q_norm = RMSNorm(heads, dim_head)
        self.k_norm = RMSNorm(heads, dim_head)
        self.dropout_p = dropout
        ...
        self.cto_q = nn.Linear(coord_dim, inner_dim, bias = False)
        self.cto_kv = nn.Linear(coord_dim, inner_dim * 2, bias = False)
    ...
    def forward(self, 
            x: torch.Tensor,
            y: torch.Tensor,
            x_pos_emb: torch.Tensor,
            y_pos_emb: torch.Tensor,
            attn_mask: Optional[torch.Tensor]=None):
        """attention rope

        Args:y
            x (torch.Tensor): (B, N, D)
            y (torch.Tensor): (B, N, D)
            x_pos_emb (torch.Tensor): (N, D)
            y_pos_emb (torch.Tensor): (B, G, N, D)
            attn_mask (Optional[torch.Tensor], optional): (*, N, M). Defaults to None.
        Raises:
            ValueError: attn_mask not valid

        Returns:
            av: (B, H, N, D)
        """
        B = x.shape[0]
        x, x_pos_emb = torch.split(
            self.norm(torch.cat([x, x_pos_emb.unsqueeze(0).expand(B, -1, -1)], dim=-1)),
            [x.shape[-1], x_pos_emb.shape[-1]], dim=-1)
        
        G = y_pos_emb.shape[1]
        q = self.to_q(x)  # (B, N, D)
        k, v = torch.chunk(self.to_kv(y), 2, dim=-1)  # (B, M, D), (B, M, D)
        cq = self.cto_q(x_pos_emb)  # (B, N, D)
        ck, cv = torch.chunk(self.cto_kv(y_pos_emb), 2, dim=-1)  # (B, G, M, D), (B, G, M, D)
        q = rearrange(q + cq, 'b n (h d) -> b h n d', h = self.heads, d = self.dim_head)  # (B, N, D), (N, D) -> (B, N, D) -> (B, h, N, d)
        k = rearrange(k.unsqueeze(1) + ck, 'b g m (h d) -> b g h m d', h = self.heads, d = self.dim_head)  # (B, 1, M, D), (B, G, M, D) -> (B, G, M, D) -> (B, G, H, M, d)
        v = rearrange(v.unsqueeze(1) + cv, 'b g m (h d) -> b g h m d', h = self.heads, d = self.dim_head)  # (B, 1, M, D), (B, G, M, D) -> (B, G, M, D) -> (B, G, H, M, d)
        q = self.q_norm(q)  # (B, N, D)
        k = self.k_norm(k)  # (B, G, M, D)
        if exists(attn_mask):  # (B, 1, N, M)
            if attn_mask.ndim == 2:
                attn_mask = attn_mask[None, None, :, :]
            elif attn_mask.ndim == 3:
                attn_mask = attn_mask[:, None, :, :]
                ...
                raise ValueError("attn_mask.ndim must be 2,3,4, got {}".format(attn_mask.ndim))
        # only supported by Pytorch 2.0+
        attn = F.scaled_dot_product_attention(
            q.unsqueeze(1).expand(-1, G, -1, -1, -1).flatten(end_dim=1),
            k.flatten(end_dim=1), v.flatten(end_dim=1),
            attn_mask = attn_mask,
            dropout_p = (self.dropout_p if self.training else 0.0),
            scale=1.0)
        attn = rearrange(attn, 'b h n d -> b n (h d)')  # (B * G, H, N, d) -> (B * G, N, H*d)
        attn = self.to_out(attn)  # (B * G, N, D) -> (B * G, N, D)
        return attn

class AttentionSplitUnNorm(nn.Module):
    def __init__(self, input_dim: int, heads: int, dim_head: int, coord_dim: int, dropout = 0., **argv):
        super().__init__()
        inner_dim = dim_head *  heads
        self.heads = heads
        ...
        self.cto_q = nn.Linear(coord_dim, inner_dim, bias = False)
        self.cto_kv = nn.Linear(coord_dim, inner_dim * 2, bias = False)
        self.coord_softmax = nn.Softmax(dim=-1)
        self.coord_dropout = nn.Dropout(dropout)
    ...
    def forward(self, 
        ...
        if exists(attn_mask):  # (B, 1, N, M)
            if attn_mask.ndim == 2:
                ...
                raise ValueError("attn_mask.ndim must be 2,3,4, got {}".format(attn_mask.ndim))
        # only supported by Pytorch 2.0+
        attn = F.scaled_dot_product_attention(q.unsqueeze(1).expand(-1, G, -1, -1, -1).flatten(end_dim=1),
            k.flatten(end_dim=1), v.flatten(end_dim=1),
            attn_mask = attn_mask,
            dropout_p = (self.dropout_p if self.training else 0.0),
            scale=1.0)
...
class AttentionELU(nn.Module):
    def __init__(self, input_dim:int, heads = 8, dim_head = 64, dropout = 0., **argv):
        ...
        self.act = lambda x: F.elu(x, alpha=1.0, inplace=True) + 1
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(
        ...
        **argv
        ...
        q = self.act(q)  # phi(Q)
        k = self.act(k)  # phi(K)
        k_sum = torch.sum(k, dim=-2, keepdim=True)  # (B, h, M, D) -> (B, h, 1, D)
        Z = 1.0 / (torch.matmul(q, k_sum.transpose(-1, -2)) + 1.0E-8)  # (B, h, N, D), (B, h, D, 1) -> (B, h, N, 1)
        dots = torch.matmul(q, k.transpose(-1, -2))  # (B, h, N, D), (B, h, D, M) -> (B, h, N, M)
...
# Modified From https://github.com/lucidrains/vit-pytorch/blob/main/vit_pytorch/na_vit.py
class AttentionSplitBkUp(nn.Module):
    def __init__(self, input_dim: int, heads: int, dim_head: int, coord_dim: int, dropout = 0., **argv):
        super().__init__()
        inner_dim = dim_head *  heads
        self.heads = heads
        self.norm = LayerNorm(input_dim)  # 注意，这里是feature通道的input_dim
        ...
        self.coord_to_v = nn.Linear(coord_dim, dim_head, bias = False)
        self.coord_softmax = nn.Softmax(dim=-1)
        self.coord_dropout = nn.Dropout(dropout)

        out_dim = inner_dim + dim_head  # coord attention takes one dim_head for encoding
        self.to_out = nn.Sequential(
            nn.Linear(out_dim, inner_dim, bias = False),
            nn.Dropout(dropout)
        )
        self.feature_av = None
    ...
    def encoder_cache(self, 
        ...
        attn = self.softmax(dots)  # (B, H, N, M)
        attn = self.dropout(attn)  # (B, H, N, M)

        return torch.matmul(attn, v)  # (B, H, N, M), (B, H, M, D) -> (B, H, N, D)
    
    def store_buffer(self, x: torch.Tensor, y: torch.Tensor, attn_mask: torch.Tensor):
        self.feature_av = self.encoder_cache(x, y, attn_mask)

    def get_buffers(self):
        return self.feature_av
    
    def clear_buffer(self):
        self.feature_av = None

    @contextmanager
    def model_buffer_manager(self, x: torch.Tensor, y: torch.Tensor, attn_mask: torch.Tensor):
        ...
        """
        try:
            self.store_buffer(x, y, attn_mask)
            yield
        finally:
            self.clear_buffer()
    ...
    def forward(
        ...
        Args:
            ...
            x_pos_emb (torch.Tensor): (N, D1)
            y_pos_emb (torch.Tensor): (B, G, M, D1), different Tcl generate different projected points, G is the number of samples
            attn_mask (Optional[torch.Tensor], optional): (B, G, N, M) to mask out valid points. Defaults to None.
        ...
        """
        G = y_pos_emb.shape[1]
        if self.feature_av is not None:
            feature_av = self.feature_av  # (B, H, N, D)
        else:
            feature_av = self.encoder_cache(x, y)  # TODO: no mask for feature cross attention
        pos_v = self.coord_to_v(y_pos_emb)  # (B, G, M, 2) -> (B, G, M, D) head expansion
        coord_dots = torch.matmul(x_pos_emb[None, None, ...], y_pos_emb.transpose(-1, -2))  # (1, 1, N, D), (B, G, D, M) -> (B, G, N, M)
        if exists(attn_mask):
            if attn_mask.ndim == 2:
                attn_mask = attn_mask[None, None, :, :]
            elif attn_mask.ndim == 3:
                ...
                raise NotImplementedError("attn_mask.ndim must be 2,3,4, got {}".format(attn_mask.ndim))
            coord_dots.masked_fill_(~attn_mask, -torch.finfo(coord_dots.dtype).max)
        
        attn = self.dropout(self.coord_softmax(coord_dots))  # (B, G, N, M) -> (B, G, N, M) 
        av = torch.matmul(attn, pos_v)  # (B, G, N, M), (B, G, M, D) -> (B, G, N, D)
        av = torch.cat([feature_av.unsqueeze(1).expand(-1, G, -1, -1, -1), av.unsqueeze(2)], dim = 2)  # (B, G, h, N, D), (B, G, 1, N, D) -> (B, G, h+1, N, D)
        av = rearrange(av, 'b g h n d -> b g n (h d)')  # head merging (B, G, h+1, N, D) -> (B, G, N, (h+1) * D)
        return self.to_out(av)  # (B, G, N, (h+1) * D) -> (B, G, N, D2)

# Modified From https://github.com/lucidrains/vit-pytorch/blob/main/vit_pytorch/na_vit.py
class AttentionSplit2(nn.Module):
    def __init__(self, input_dim: int, heads: int, dim_head: int, coord_dim: int, dropout = 0.):
        super().__init__()
        inner_dim = dim_head *  heads
        self.heads = heads
        self.norm = LayerNorm(input_dim)  # 注意，这里是feature通道的input_dim
        ...
        self.coord_q_norm = nn.LayerNorm(dim_head)
        self.coord_k_norm = nn.LayerNorm(dim_head)
        self.coord_to_q = nn.Linear(coord_dim ,dim_head, bias= False)
        self.coord_to_kv = nn.Linear(coord_dim, 2 * dim_head, bias= False)
        self.coord_softmax = nn.Softmax(dim=-1)
        self.coord_dropout = nn.Dropout(dropout)
    ...
    def forward(
        ...
        if self.feature_av is not None:
            ...
            feature_av = self.encoder_cache(x, y)  # TODO: no mask for feature cross attention
        coord_q = self.coord_to_q(x_pos_emb)  # (B, G, N, D)
        coord_k, coord_v = torch.chunk(self.coord_to_kv(y_pos_emb), 2, dim=-1)  # (B, G, N, D), (B, G, N, D)
        coord_q = self.coord_q_norm(coord_q)
        coord_k = self.coord_k_norm(coord_k)
        coord_dots = torch.matmul(coord_q[None, None, ...], coord_k.transpose(-1, -2))  # (1, 1, N, D), (B, G, D, M) -> (B, G, N, M)
        if exists(attn_mask):
            if attn_mask.ndim == 2:
                attn_mask = attn_mask[None, None, :, :]
            elif attn_mask.ndim == 3:
        ...
        attn = self.dropout(self.coord_softmax(coord_dots))  # (B, G, N, M) -> (B, G, N, M) 
        av = torch.matmul(attn, coord_v)  # (B, G, N, M), (B, G, M, D) -> (B, G, N, D)
        av = torch.cat([feature_av.unsqueeze(1).expand(-1, G, -1, -1, -1), av.unsqueeze(2)], dim = 2)  # (B, G, h, N, D), (B, G, 1, N, D) -> (B, G, h+1, N, D)
        av = rearrange(av, 'b g h n d -> b g n (h d)')  # head merging (B, G, h+1, N, D) -> (B, G, N, (h+1) * D)
        return self.to_out(av)  # (B, G, N, (h+1) * D) -> (B, G, N, D2)
...
__ATTENTION_TYPE__ = {
    "Attention": Attention,
    "AttentionUnNorm": AttentionUnNorm,
    "AttentionELU": AttentionELU,
    "AttentionRoPE": AttentionRoPE,
    "AttentionELURoPE": AttentionELURoPE,
    "AttentionSplit": AttentionSplit,
    "AttentionSplitUnNorm": AttentionSplitUnNorm
}

__ATTENTION__ = TypeVar("__ATTENTION__", Attention, AttentionUnNorm, AttentionELU, AttentionRoPE, AttentionELURoPE, AttentionSplit, AttentionSplitUnNorm)