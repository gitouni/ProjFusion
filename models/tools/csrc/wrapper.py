...
def furthest_point_sampling(xyz: torch.Tensor, n_samples: int, cpp_impl=True):
    ...
    def _furthest_point_sampling_py(_xyz: torch.Tensor, _n_samples: int):
        ...
        _n_samples = min(n_points, _n_samples)
...
def wrapped_fps(xyz:torch.Tensor, n_samples:int, cpp_impl=True):
    """wrapped furthest point sampling

    Args:
        xyz (torch.Tensor): (B, 3, N)
        n_samples (int): G
        cpp_impl (bool, optional): use cuda C++. Defaults to True.

    Returns:
        _type_: B 3 N
    """
    center_idx = furthest_point_sampling(xyz.transpose(-1,-2), n_samples, cpp_impl)  # B G
    center_idx = center_idx[:,None,:].repeat(1,3,1)  # B 3 G
    return torch.gather(xyz, dim=-1, index=center_idx)  # B 3 N
...
def k_nearest_neighbor(input_xyz: torch.Tensor, query_xyz: torch.Tensor, k: int, cpp_impl=True):
    ...
    if cpp_impl and callable(_k_nearest_neighbor_cuda) and input_xyz.is_cuda:
        return _k_nearest_neighbor_cuda(input_xyz.contiguous().float(), query_xyz.contiguous().float(), k)
    else:
