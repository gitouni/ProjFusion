...
from ..util.constant import BatchedCameraInfoDict
...
def build_pc_pyramid(pc1:torch.Tensor, pc2:torch.Tensor, n_samples_list:List[int]):
...
def build_pc_pyramid_single(pc1:torch.Tensor, n_samples_list:List[int]):
    batch_size, _, n_points = pc1.shape

    # sub-sampling point cloud
    sample_index = furthest_point_sampling(pc1.transpose(1, 2), max(n_samples_list))  # 1/4
    # build point cloud pyramid
    lv0_index = torch.arange(n_points, device=pc1.device)
    lv0_index = lv0_index[None, :].expand(batch_size, n_points)
    xyzs, sample_indices = [pc1], [lv0_index]

    for n_samples in n_samples_list:  # 1/4
        sample_indices.append(sample_index[:, :n_samples])
        xyzs.append(batch_indexing(pc1, sample_index[:, :n_samples]))

    return xyzs, sample_indices
...
def project_pc2image(pc:torch.Tensor, camera_info:BatchedCameraInfoDict):
    # pc: (B, G, 3, N) or (B, 3, N)
    ...
    if camera_info['projection_mode'] == 'perspective':
        ...
        raise NotImplementedError("projection_mode can only be 'perspective' or 'parallel', got {}".format(camera_info['projection_mode']))

    return torch.stack([image_x, image_y], dim=-2)  # (B, 2, N) or (B, K, 2, N)
